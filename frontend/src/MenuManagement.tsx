      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            Manage pricing, availability, and inventory with instant state updates.
          </p>
        </div>
          ...
          <div className="bg-white rounded-[32px] p-6 border border-slate-100 shadow-sm flex flex-col">
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`h-1.5 w-1.5 rounded-full ${active ? 'bg-blue-500' : 'bg-slate-200'}`} />
                        <div>
                          <div className={`text-[15px] font-bold ${active ? 'text-[#0c1424]' : 'text-slate-600'}`}>
                            {category.name}
                          </div>
                    ...
                    <div className="flex items-center justify-between px-4 pb-4">
                      ...
                      >
                        <div className={`absolute top-1 left-1 w-5 h-5 rounded-full bg-white transition-transform duration-200 ${category.isActive ? 'translate-x-5' : 'translate-x-0'}`} />
                      </button>
                      <span className="text-[11px] font-bold text-slate-400">Items follow this state in POS</span>
                    </div>
                  </div>
                );
              })}
            ...
            {isCreatingCategory ? (
              <div className="mt-6 rounded-2xl border border-slate-100 bg-slate-50/60 p-4 space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">New Category</label>
                <input
                  type="text"
        ...
        <div className="col-span-12 lg:col-span-8 bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-50 bg-slate-50/30">
                  <th className="text-left py-6 px-8 text-[10px] font-black text-slate-400 uppercase tracking-widest w-1/2">Item Details</th>
                  <th className="text-left py-6 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Price (AUD)</th>
                  <th className="text-left py-6 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Track Inv</th>
                  <th className="text-left py-6 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Active</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredItems.map((item) => {
                  ...
                  const effectiveActive = Boolean(category?.isActive) && item.isActive;
                  ...
                  return (
                    <tr key={item.id} className="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <div className="flex items-center gap-5">
                          ...
                          <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <span className="bg-blue-50 text-[9px] font-black text-blue-600 px-2 py-0.5 rounded-full uppercase tracking-widest">{category?.name || 'Uncategorized'}</span>
                              {!effectiveActive && (
                                <span className="bg-rose-50 text-[9px] font-black text-rose-600 px-2 py-0.5 rounded-full uppercase tracking-widest">Inactive</span>
                              )}
                            </div>
                      ...
                      <td className="py-6 px-4 text-center">
                        <span className="text-[15px] font-black text-[#0c1424]">${item.price.toFixed(2)}</span>
                      </td>
                      <td className="py-6 px-4 text-center">
                        <div className="flex justify-center">
                          ...
                          >
                            <div className={`absolute top-1 left-1 w-5 h-5 rounded-full bg-white transition-transform duration-200 ${item.trackInventory ? 'translate-x-5' : 'translate-x-0'}`} />
                          </button>
                        </div>
                      </td>
                      <td className="py-6 px-4 text-center">
                        <div className="flex justify-center">
                          ...
                          >
                            <div className={`absolute top-1 left-1 w-5 h-5 rounded-full bg-white transition-transform duration-200 ${effectiveActive ? 'translate-x-5' : 'translate-x-0'}`} />
                          </button>
                        </div>
                      </td>
                      <td className="py-6 px-8">
          ...
          <div className="p-8 border-t border-slate-50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button disabled className="h-9 w-9 rounded-xl border border-slate-100 flex items-center justify-center text-slate-300">
                <ChevronDown size={16} className="rotate-90" />
              </button>
              <button className="h-9 w-9 rounded-xl bg-[#0c1424] text-white text-xs font-black flex items-center justify-center">1</button>
              <button className="h-9 w-9 rounded-xl border border-slate-100 text-slate-600 text-xs font-bold hover:bg-slate-50 flex items-center justify-center transition-colors">2</button>
